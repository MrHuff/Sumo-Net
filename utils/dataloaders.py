...
class IdentityTransformer(BaseEstimator,
                          TransformerMixin):  # Scaling is already good. This leaves network architecture...
    def __init__(self):
        pass
...
class survival_dataset(Dataset):
    def __init__(self, str_identifier, seed=1337, fold_idx=0, sumo_net=True, conformal=False):
        print('fold_idx: ', fold_idx)
        super(survival_dataset, self).__init__()
        if str_identifier in ['support']:
            data = support
            ...
            data = kkbox
            cont_cols = ['n_prev_churns', 'log_days_between_subs', 'log_days_since_reg_init', 'log_payment_plan_days',
        ...
        df_train, df_val, _, _ = train_test_split(df_train, df_train[self.event_col], test_size=0.25,
                                                  stratify=df_train[self.event_col])
        ...
        y_train = self.duration_mapper.fit_transform(df_train[self.duration_col].values.reshape(-1, 1)).astype(
            'float32')
        ...
        self.y_train_ref = self.duration_mapper_2.fit_transform(
            df_train[self.duration_col].values.reshape(-1, 1)).astype('float32')
        self.y_val_ref = self.duration_mapper_2.transform(df_val[self.duration_col].values.reshape(-1, 1)).astype(
            'float32')
        self.y_test_ref = self.duration_mapper_2.transform(df_test[self.duration_col].values.reshape(-1, 1)).astype(
            'float32')
        ...
        if conformal:
            df_conformal, df_val, _, _ = train_test_split(df_val, df_val[self.event_col], test_size=0.5,
                                                          stratify=df_val[self.event_col])
            x_conformal = self.x_mapper.transform(df_conformal[cont_cols + binary_cols]).astype('float32')
            y_conformal = self.duration_mapper.transform(df_conformal[self.duration_col].values.reshape(-1, 1)).astype(
                'float32')
            self.y_conformal_ref = self.duration_mapper_2.transform(
                df_conformal[self.duration_col].values.reshape(-1, 1)).astype('float32')
            self.split(X=x_conformal, delta=df_conformal[self.event_col], y=y_conformal, mode='conformal', cat=cat_cols,
                       df=df_conformal)
...
class chunk_iterator():
    ...
    def __next__(self):
        ...
        if self._index < self.true_chunks:
            if self.valid_cat:
                result = (
                self.it_X[self._index], self.it_cat_X[self._index], self.it_y[self._index], self.it_delta[self._index])
            else:
...
def get_dataloader(str_identifier, bs, seed, fold_idx, shuffle=True, sumo_net=False, conformal=False):
    d = survival_dataset(str_identifier, seed, fold_idx=fold_idx, sumo_net=sumo_net, conformal=conformal)
